		...
		<div class="row ">
			<div class="col-12">
				<form class="form-horizontal" method="POST" action="/pengumuman/{{ $action }}{{($action!='simpan')? '/'.$pengumuman['id'] : ''}}">
					...
					<h4 class="page-head-line">Form Edit Pengumuman</h4>
					...
					<div class="modal-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-8">
								<textarea name="isi" style="width: 753px; height: 140px;"><?php echo $pengumuman['isi'];?></textarea></td></tr>
							</div>
						</div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                    ...
                                    @foreach($kelas as $category)
                                     <option value="{{ $category->id }}"  @if($category->id==$pengumuman->kelas_id) selected='selected' @endif >{{ $category->nama_kelas }}</option>
                                    @endforeach 
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-8">
                                    ...
                                    @foreach($angkatan as $category)
                                     <option value="{{ $category->id }}"  @if($category->id==$pengumuman->angkatan_id) selected='selected' @endif >{{ $category->nama_angkatan }}</option>
                                    @endforeach 
                                </select>
                            </div>
                        </div>                  												
