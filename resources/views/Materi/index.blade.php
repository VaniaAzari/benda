		...
		<div class="col-12">
			<div class="panel panel-default">
				<div class="panel-heading text-right">
					 ...
					 @endforeach
					<a href="/materi/create/{{$value['matakuliah_id']}}/{{$value['kelas_id']}}/{{$value['angkatan_id']}}" class="btn btn-md btn-info" >
				...
				<table id="dataTable" class="table table-bordered table-striped table-hover">
					<thead>
						<tr>
							...
							<th>Matakuliah</th>
							<th>Kelas</th>
							<th style="width: 450px;">Konten</th>							
							<th>Aksi</th>
						</tr>
					</thead>		
					<tfoot>			
...
<script>
    $( document ).ready(function() {
        var dt = $('#dataTable').DataTable({
            ...
            autoWidth: false,
            ajax: {
                url :'{{ route('materi.getdata', ['matakuliah_id' => $matakuliah_id,'kelas_id' => $kelas_id,'angkatan_id' => $angkatan_id]) }}',
         ...
         $('table#dataTable tbody').on( 'click', 'td>a', function (e) {
            ...
            if($(this).hasClass('delete')) {
                swal({
                    ...
                    value == 'execute' ? deleteData(data.id, $('input[name="_token"]').val(), "{{ route('materi.delete') }}")
                    : null
                });
